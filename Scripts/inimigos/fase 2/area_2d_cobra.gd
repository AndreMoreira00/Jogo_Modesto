...
func _process(delta: float) -> void:
	if player:
		if player.global_position.x-50 <= global_position.x:
			$AnimatedSprite2D.scale.x = 1  
		elif player.global_position.x-50 >= global_position.x:
			$AnimatedSprite2D.scale.x = -1
	...
	if $AnimatedSprite2D.animation != "morrer" and exit and $AnimatedSprite2D.animation == "atacar" and $AnimatedSprite2D.frame == 6:
...
func _on_frame_changed():
	if $AnimatedSprite2D.frame == 3 and $AnimatedSprite2D.animation == "atacar":
		$HitBox/HitBox.disabled = false
	elif $AnimatedSprite2D.frame == 0 and $AnimatedSprite2D.animation == "atacar":
